...
int32_t sys_rmdir(const char* pathname)
{
    ...
    if (inode_no == -1) {
        printk("In %s, subpath %s does not exist\n", pathname, searched_record.searched_path);
    } else {
        if (searched_record.file_type == FT_REGULAR) {
            printk("%s is regular file!\n", pathname);
        } else {
            ...
            if (!dir_is_empty(dir)) {
                printk("dir %s is not empty, it is not allowd to delete a nonempty directory\n", pathname);
            } else {
...
static uint32_t get_parent_dir_inode_num(uint32_t child_inode_num, void* io_buf)
{
    struct inode* child_dir_inode = inode_open(cur_part, child_inode_num);
...
static int get_child_dir_name(uint32_t p_inode_num, uint32_t c_inode_num, char* path, void* io_buf)
{
    struct inode* parent_dir_inode = inode_open(cur_part, p_inode_num);
    ...
    while (block_idx < block_cnt) {
        if (all_blocks[block_idx]) {
            ...
            while (dir_e_idx < dir_entrys_per_sec) {
                if ((dir_e + dir_e_idx)->i_no == c_inode_num) {
                    strcat(path, "/");
...
char* sys_getcwd(char* buf, uint32_t size)
{
    ...
    int32_t child_inode_num = cur_thread->cwd_inode_num;
    ASSERT(child_inode_num >= 0 && child_inode_num < 4096);
    ...
    while (child_inode_num) {
        parent_inode_num = get_parent_dir_inode_num(child_inode_num, io_buf);
        if (get_child_dir_name(parent_inode_num, child_inode_num,
...
int32_t sys_chdir(const char* path)
{
    ...
    if (inode_no != -1) {
        if (searched_record.file_type == FT_DIRECTORY) {
            running_thread()->cwd_inode_num = inode_no;
            ret = 0;
        } else {
