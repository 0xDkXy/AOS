...
int32_t get_free_slot_in_global(void)
{
    ...
    if (fd_idx == MAX_FILE_OPEN) {
        printk("exceed max open files limit");
        return -1;
    }
    return fd_idx;
}
...
int32_t pcb_fd_install(int32_t global_fd_idx)
{
    ...
    if (local_fd_idx == MAX_FILES_OPEN_PER_PROC) {
        printk("exceed max open files_per_proc limit");
        return -1;
    }
    return local_fd_idx;
}
...
int32_t file_write(struct file* file, const void* buf, uint32_t count)
{
    ...
    if (add_blocks == 0) {
        ...
        if (file_will_use_blocks <= 12) {
            ...
            while (block_idx < file_will_use_blocks) {
                block_lba = block_bitmap_alloc(cur_part);
                if (block_lba == -1) {
                    printk("file_write: block_bimap_alloc for situation 1 failed\n");
                    return -1;
                }
                ASSERT(file->fd_inode->i_sectors[block_idx] == 0);
    ...
    // printk("1: bytes_written: %d\n", bytes_written);
    while (bytes_written < count) {
        memset(io_buf, 0, BLOCK_SIZE);
        sec_idx = file->fd_inode->i_size / SECTOR_SIZE;
        ...
        // printk("2: bytes_written: %d\n", bytes_written);
    }
    inode_sync(cur_part, file->fd_inode, io_buf);
    sys_free(all_blocks);
    sys_free(io_buf);
    // printk("3: bytes_written: %d\n", bytes_written);
